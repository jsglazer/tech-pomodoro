...
- **The countdown is the control.** Click the numbers in the popover to pause or resume; dedicated Stop and Pause buttons sit beneath them.
- **Menu bar as a countdown.** Show minutes remaining or a clock icon, optionally on a filled background. During Work the minutes turn yellow at a warning threshold and red at an alert threshold — both configurable, either one disabled by setting it to 0.
- **Alerts.** A system-sound ding and a menu bar flash at every interval, cycle, and session boundary — each independently switchable. Sounds are read from this Mac's sound folders, so the picker never offers something that will not play.
...
- **Analytics.** Cycles completed and total work time for Today, the last 3 days, and the last 5 days, as calendar days in your timezone. Hover the panel to refresh it; the numbers are visible either way.
